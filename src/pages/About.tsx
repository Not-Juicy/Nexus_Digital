...
export default function About() {
  ...
  const clients = ['TechCorp', 'SaaS Labs', 'Apex Digital', 'NextGen', 'Pixel Studios', 'CloudBase'];

  const team = [
    { name: 'Dan P.', role: 'Founder & Lead Strategist', desc: '10+ years architecting digital growth engines across SEA markets.' },
    { name: 'Sofia L.', role: 'Head of Operations', desc: 'Scaling systems from zero to enterprise-grade with precision.' },
    { name: 'Marcus K.', role: 'AI & Automation Lead', desc: 'Building intelligent pipelines that eliminate manual overhead.' },
  ];
  ...
  return (
    <div className="pt-32 bg-black">
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto border-b border-white/10 relative overflow-hidden">
        ...
        <div className="absolute top-[-10%] right-[-5%] w-[40%] h-[60%] bg-[radial-gradient(circle_at_70%_30%,rgba(220,38,38,0.05)_0%,transparent_70%)]" />
        <div className="relative z-10">
          ...
          >
            <span className="inline-block">YOUR DIGITAL</span>{' '}
            <br className="hidden md:block" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-800 italic">PARTNER</span>{' '}
            <span className="inline-block">FOR GROWTH.</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {values.map((v, i) => (
              <motion.div
                ...
                className="p-8 bg-[#111] border border-white/5 hover:border-red-500/30 transition-all duration-500 group hover:translate-y-[-4px] hover:shadow-[0_8px_30px_-4px_rgba(220,38,38,0.1)]"
              >
                <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center mb-6 group-hover:bg-red-600 transition-colors duration-500">
      ...
      <section className="py-20 bg-[#080808] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] text-center mb-10">Trusted by forward-thinking teams</p>
          <div className="flex flex-wrap justify-center gap-x-16 gap-y-8">
            {clients.map((name, i) => (
              <motion.div
                ...
                className="text-white/10 text-2xl font-black tracking-tighter italic hover:text-white/30 transition-colors"
              >
                {name}
              </motion.div>
            ))}
      ...
      <section className="py-32 bg-[#080808] border-t border-white/5 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-[-5%] text-[20vw] font-black text-white/[0.012] select-none pointer-events-none tracking-tighter italic leading-none">METRICS</div>
                ...
                >
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                      <skill.icon className="w-4 h-4 text-red-600" />
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto border-t border-white/10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-12">
          <div>
            <span className="text-red-500 font-mono text-[10px] font-bold uppercase tracking-[0.4em] mb-4 block underline">Leadership</span>
            <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase italic leading-none">
              THE PEOPLE <span className="text-red-600">BEHIND IT.</span>
            </h2>
          </div>
          <p className="max-w-sm text-white/40 text-sm leading-relaxed uppercase tracking-wide">
            A lean crew of seasoned operators who have shipped at scale.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {team.map((member, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ delay: i * 0.1 }}
              className="group relative bg-[#0a0a0a] p-10 border border-white/5 hover:border-red-500/30 transition-all duration-500"
            >
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-red-600 to-red-900 flex items-center justify-center mb-8 text-2xl font-black text-white group-hover:scale-110 transition-transform duration-500">
                {member.name.charAt(0)}
              </div>
              <h3 className="text-2xl font-black text-white mb-1 tracking-tight">{member.name}</h3>
              <p className="text-red-500 text-[10px] font-bold uppercase tracking-[0.3em] mb-6">{member.role}</p>
      ...
      <section id="process" className="py-32 px-6 max-w-7xl mx-auto border-t border-white/10">
        ...
        <div className="relative">
          <div className="hidden lg:block absolute top-24 left-[12.5%] right-[12.5%] h-px bg-gradient-to-r from-transparent via-red-600/30 to-transparent" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {processSteps.map((step, i) => (
              ...
              >
                ...
                <div className="relative z-10">
                  <span className="text-6xl font-black text-white/[0.04] group-hover:text-red-600/20 transition-colors absolute top-2 right-4 leading-none">{step.phase}</span>
                  <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center mb-8 group-hover:bg-red-600 transition-colors relative z-10">
                    <step.icon className="w-6 h-6 text-white" />
                  </div>
                  <span className="text-[10px] font-black text-red-500 uppercase tracking-[0.3em] block mb-2 relative z-10">Step {step.phase}</span>
                  <h3 className="text-2xl font-black text-white mb-2 tracking-tight group-hover:text-red-500 transition-colors relative z-10">{step.title}</h3>
                  <p className="text-white/30 text-[10px] font-bold uppercase tracking-[0.3em] mb-4 relative z-10">{step.subtitle}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {caseStudies.map((cs, i) => (
              <Link key={cs.slug} to={`/case-studies/${cs.slug}`} className="group block">
                ...
                >
                  <div className="aspect-[16/10] overflow-hidden bg-[#111] border border-white/5 relative mb-8 rounded-lg">
                    <img
                      src={cs.image}
                      alt={cs.title}
